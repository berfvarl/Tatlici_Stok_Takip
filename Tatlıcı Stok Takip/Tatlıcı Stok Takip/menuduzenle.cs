...
namespace Tatlıcı_Stok_Takip
{
    public partial class menuduzenle : Form
    {
        ...
        {
            baglanti.Open();
            OleDbCommand guncellemeKomutu = new OleDbCommand("update menu set barkodno='" + textBox1.Text + "', kategori='" + comboBox1.Text + "', urunturu='" + comboBox2.Text + "', urun='" + comboBox3.Text + "', menuadi='" + textBox2.Text + "', adet='" + textBox3.Text + "', rafno='" + textBox4.Text + "', gtarihi='" + dateTimePicker1.Text + "' where barkodno='" + textBox1.Text+"'", baglanti);
            guncellemeKomutu.ExecuteNonQuery();
            baglanti.Close();
            label9.Text = "Ürün Güncellendi.";
        }
    }
}
